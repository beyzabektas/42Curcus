...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/19 19:25:48 by bebektas          #+#    #+#             */
/*   Updated: 2023/09/19 22:15:14 by bebektas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (read_size > 0)
	{
		...
		if ((read_size <= 0 && !stash[fd]) || read_size == -1)
			return (ft_free_stash(&stash[fd], 0));
		buf[read_size] = '\0';
		stash[fd] = backup_stash(stash[fd], buf);
		if (check_newline(stash[fd]))
		{
			line = extract_line(stash[fd]);
			if (!line)
				return (ft_free_stash(&stash[fd], 0));
			return (stash[fd] = remain_stash(stash[fd]), line);
		}
	}
